...
- Place entries under a concrete Semantic Versioning release section, in an existing `Added`, `Changed`, `Fixed`, `Removed`, or similar subsection where possible.
...
- When reviewing a PR, flag touched entries missing the relevant PR reference, including entries in nested subsections such as `Developer API` / `C++` / `Added`. A reference to an issue alone does not satisfy the PR-reference requirement.
