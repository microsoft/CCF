...
- Flag any non-ASCII characters introduced in source code, comments, docstrings, or string literals (for example em/en dashes `—`/`–`, smart quotes `“”`/`‘’`, arrows `→`, or ligatures such as `ﬁ`). These frequently slip in via copy-paste or AI-generated text. Ask the author to replace them with the plain ASCII equivalent (for example `—` → `-`, `→` → `->`, `“` → `"`). Genuinely intentional non-ASCII (such as box-drawing or block glyphs used to render terminal charts/visualisations, or characters that are the subject of a test) is acceptable; everything else should be ASCII.
