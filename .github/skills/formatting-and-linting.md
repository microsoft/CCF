...
| `scripts/ascii-checks.sh`        | `scripts/ascii-checks.sh`        | `.h`, `.hpp`, `.c`, `.cpp`, `.py` (excludes `3rdparty/` and intentionally non-ASCII files) | Non-ASCII characters that must be replaced with their plain ASCII equivalents                                                          |
