    ...
    with infra.ccf.network(
        ...
        with primary.node_client() as mc:
            ...
            with primary.user_client(format="json") as c:
                check_commit(c.rpc("LOG_record", {"id": 42, "msg": msg}), result=True, force_sign=True)
                check_commit(c.rpc("LOG_record", {"id": 43, "msg": msg2}), result=True, force_sign=True)
            ...
            with primary.user_client(format="json") as c:
                ...
                for p in range(10, 13):
                    long_msg = "X" * (2 ** p)
                    check_commit(
                        c.rpc("LOG_record", {"id": id, "msg": long_msg}), result=True, force_sign=True
                    )
