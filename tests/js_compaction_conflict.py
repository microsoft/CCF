...
def test_compaction_conflict_is_retried(network, args):
    ...
    # The map must already be known to the store at the spinning transaction's
    # read version. A map which does not yet exist is created by the
    # transaction instead, which does not conflict.
