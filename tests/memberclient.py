...
def test_governance(network, args):
    ...
    assert (
        network.consortium.get_member_by_local_id(0)
        .vote(node, new_member_proposal, careful_vote)
        .status_code
        == params_error
    )
    assert (
        network.consortium.get_member_by_local_id(1)
        .vote(node, new_member_proposal, careful_vote)
        .status_code
        == params_error
    )
    ...
    response = network.consortium.get_member_by_local_id(1).withdraw(node, proposal)
