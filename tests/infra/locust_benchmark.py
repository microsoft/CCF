...
def run_locust(
    ...
    # Locust workers inherit the master's affinity, so prefixing the master is
    # enough to keep the whole client side off the node's CPUs.
    subprocess.run(
        infra.cpu_isolation.command_prefix(infra.cpu_isolation.CLIENT) + cmd,
        check=True,
