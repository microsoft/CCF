...
def test_metrics(network, args):
    ...
    with primary.client("user0") as c:
        ...
        m = get_metrics(r, "endpoint_metrics", "GET")
        calls = m["calls"]
        errors = m["errors"]

    with primary.client("user0") as c:
        r = c.get("/app/endpoint_metrics")
        assert get_metrics(r, "endpoint_metrics", "GET")["calls"] == calls + 1
        r = c.get("/app/endpoint_metrics")
        assert get_metrics(r, "endpoint_metrics", "GET")["calls"] == calls + 2
    ...
    with primary.client() as c:
        r = c.get("/app/endpoint_metrics")
        assert get_metrics(r, "endpoint_metrics", "GET")["errors"] == errors + 1
