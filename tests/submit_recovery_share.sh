...
function cleanup() {
  rm -f member_enc_privk.raw network_enc_pubk.raw
}
trap cleanup 0
...
openssl asn1parse -in "${member_enc_privk}" -strparse "${der_header_privk_len}" -out member_enc_privk.raw -noout

# Parse raw public key generated by network
der_header_pubk_len=9
openssl asn1parse -in "${network_enc_pubk}" -i -strparse "${der_header_pubk_len}" -out network_enc_pubk.raw -noout

# step CLI base64 standard to URL encoding for input
encrypted_share_b64_url=$(echo "${encrypted_share}" | sed 's/+/-/g; s/\//_/g')

# Decrypt encrypted share with nonce, member private key and defunct network public key
decrypted_share=$(echo "${encrypted_share_b64_url}" | step crypto nacl box open base64:"${nonce}" network_enc_pubk.raw member_enc_privk.raw | openssl base64 -A)

# Finally, submit decrypted share
curl -i -sS --fail https://"${node_rpc_address}"/members/recovery_share/submit "${@}" -H "Content-Type: application/json" -d '{"recovery_share": "'"${decrypted_share}"'"}'