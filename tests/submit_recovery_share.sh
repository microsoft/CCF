...
echo "${encrypted_share_b64_url}" | step crypto nacl box open base64:"${nonce}" network_enc_pubk.raw member_enc_privk.raw | openssl base64 -A | curl -i -sS --fail https://"${node_rpc_address}"/members/recovery_share/submit "${@}" -d @-