...
resp=$(curl --fail -sS -X GET https://"${node_rpc_address}"/members/recovery_share "${@}")
...
# step CLI base64 standard to URL encoding
...
curl -sS --fail https://"${node_rpc_address}"/members/recovery_share/submit "${@}" -H "Content-Type: application/json" -d '{"recovery_share": "'"${decrypted_share}"'"}'