...
export function multi_auth(request) {
  ...
  if (request.caller.policy === "user_cert") {
    ...
    lines.push(`The caller's user data is: ${JSON.stringify(request.caller.data)}`);
    ...
    lines.push("JWT");
    lines.push(`The caller is identified by a JWT issued by: ${request.caller.jwt.key_issuer}`)
    lines.push(`The JWT header is:\n${JSON.stringify(request.caller.jwt.header)}`);
    lines.push(`The JWT payload is:\n${JSON.stringify(request.caller.jwt.payload)}`);
  }
