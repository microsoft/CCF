...
`scripts/import_proofs.py SOURCE_REPOSITORY CCFRaft/Proofs` reproduces the initial
safety-only import from commit `9be0b7352`. It excludes standalone retirement
guarantees and refuses to overwrite repaired files. Later proof repairs are
maintained Lean source, not generated by that importer.

`scripts/omit_unused_sections.py BUILD_LOG PROOF_FILE` applies the compiler's
unused-instance hints to one proof file. It leaves theorem statements and tactic
bodies unchanged and rejects stale declaration locations. It does not disable
linters or discharge proof obligations.
