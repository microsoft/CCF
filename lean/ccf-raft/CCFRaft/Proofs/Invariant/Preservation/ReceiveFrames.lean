...
lemma appendRequestMemAfterVoteRequestReceive
    ...
    : forall queuedDestination queuedRequest,
        Message.appendEntriesRequest queuedRequest
          ∈ enqueue
              ...
              queuedDestination
        ↔ Message.appendEntriesRequest queuedRequest ∈ network queuedDestination := by
...
lemma voteRequestMemBackAfterVoteRequestReceive
    (network : Node -> List (Message Node TxId))
    (source destination : Node)
    (request : RequestVoteRequest Node)
    (response : RequestVoteResponse Node)
...
lemma oldAppendResponseMemAfterAppendRequestReceive
    (network : Node -> List (Message Node TxId))
    (source destination : Node)
    (request : AppendEntriesRequest Node TxId)
    (response queuedResponse : AppendEntriesResponse Node)
    ...
    (taken
      : Selected source (network destination) (.appendEntriesRequest request) remaining)
    (queuedDestination : Node)
    (member : Message.appendEntriesResponse queuedResponse ∈ network queuedDestination)
    : Message.appendEntriesResponse queuedResponse
...
lemma effectiveAckersAfterVoteRequestReceive
    (state after : View Node TxId)
    (source destination : Node)
    (request : RequestVoteRequest Node)
    (response : RequestVoteResponse Node)
...
lemma effectiveElectionVotersBeforeSubsetAfterVoteRequestReceive
    (state after : View Node TxId)
    (source destination : Node)
    (request : RequestVoteRequest Node)
    (response : RequestVoteResponse Node)
...
-/
lemma effectiveElectionVotersAfterGrantedRequestSubsetPotential
    (state : View Node TxId)
    (source destination : Node)
    (request : RequestVoteRequest Node)
    (nextNode : NodeState Node TxId)
  ...
  rcases member with ⟨joined, processed | queued⟩
  · refine ⟨by simpa [after] using joined, Or.inl ?_⟩
    simp only [
      effectiveElectionVoters, Finset.mem_filter]
    rw [votesEq candidate] at processed
    exact ⟨by simpa [after] using joined, Or.inl processed⟩
  · refine ⟨by simpa [after] using joined, ?_⟩
    rcases queued with
    ...
    · rcases new with ⟨candidateEq, responseEq⟩
      ...
      have canonicalUpToDate :
        ...
        simpa [voteLogUpToDate, makeRequestVoteRequest,
          ...
          lastCommittableTerm_eq_maxCommittableTerm
            (state.nodes request.source)
            (invariantFactsCommittedFrontierIsSignatureFromCommitEvidence
              facts request.source), sourceTerm, grantFacts.1]
      ...
      simpa [makeRequestVoteRequest, candidateEqSource, voterEqDestination]
        using And.intro candidateVoterTerm (And.intro canonicalUpToDate grantFacts.2.2.1)

/-- A post-grant effective quorum was already a pre-state prospective quorum. -/
lemma effectiveElectionMajorityAfterGrantedRequestWasPotential
    (state : View Node TxId)
    (source destination : Node)
    (request : RequestVoteRequest Node)
    (nextNode : NodeState Node TxId)
  ...
  have post := handleRequestVoteRequestLocalPost handled
  have activeConfigurationsEq :
    ...
    · subst candidate
      simp [updateNode, post.logUnchanged, post.commitIndexUnchanged]
  ...
  exact hasConfigurationMajority_mono subset
    (of_decide_eq_true
      (majority configuration (by simpa [activeConfigurationsEq] using active)))
