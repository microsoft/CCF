...
def node? (nodes : NodeStore Node TxId) (node : Node) : Option (NodeState Node TxId) :=
  nodes.entries.lookup node

/-- Read a node's state, using `freshNodeState` for an unallocated node. -/
def get (nodes : NodeStore Node TxId) (node : Node) : NodeState Node TxId :=
...
instance (nodes : NodeStore Node TxId) (node : Node) : Decidable (nodes.allocated node) :=
  inferInstanceAs (Decidable (nodes.node? node |>.isSome))

/-- Insert or replace the state of one node. -/
def set (nodes : NodeStore Node TxId) (node : Node) (value : NodeState Node TxId)
    : NodeStore Node TxId :=
...
def ofFinset (keys : Finset Node) (value : Node -> NodeState Node TxId)
    : NodeStore Node TxId :=
  let entries := keys.1.map fun node => Sigma.mk node (value node)
  ⟨{
    entries
    nodupKeys := by
...
def allocate (nodes : NodeStore Node TxId) (added : Finset Node) : NodeStore Node TxId :=
...
def State.node? (state : State Node TxId) (node : Node) : Option (NodeState Node TxId) :=
...
instance (state : State Node TxId) (node : Node) : Decidable (state.allocated node) :=
  inferInstanceAs (Decidable (state.nodes.allocated node))

/-- Replace the state of one node. -/
def updateNode (nodes : NodeStore Node TxId) (node : Node) (value : NodeState Node TxId)
    : NodeStore Node TxId :=
...
-/
def plausibleSuccessor (state : State Node TxId) (source destination : Node) : Prop :=
...
instance (state : State Node TxId) (source destination : Node)
...
-/
def candidateTransitionEnabled (state : State Node TxId) (node : Node) : Prop :=
...
def takeFirstFrom (source : Node)
    : List (Message Node TxId) -> Option (Message Node TxId × List (Message Node TxId))
  | [] => none
  | message :: tail =>
...
def takeOccurrenceFrom (source : Node)
...
def makeRequestVoteRequest (state : State Node TxId) (source destination : Node)
...
def makeRequestPreVote (state : State Node TxId) (source destination : Node)
    : RequestPreVote Node :=
  let sourceState := state.nodes source
  {
    term := sourceState.currentTerm
...
def makeProposeVoteRequest (state : State Node TxId) (source destination : Node)
...
def messageSourceAllowed (state : State Node TxId) (message : Message Node TxId) : Prop :=
  match message with
...
instance (state : State Node TxId) (message : Message Node TxId)
...
-/
def newerMessage? (state : State Node TxId) (source destination : Node)
...
def handleReceive? (state : State Node TxId) (source destination : Node)
  ...
  | some (message, remaining) =>
      if message.destination != destination then
        ...
        | .appendEntriesRequest request =>
            ...
            | some nextNode =>
                some { state with nodes := updateNode state.nodes destination nextNode }
            | none =>
        ...
        | .appendEntriesResponse response =>
            if state.allocated response.source then
              match handleAppendEntriesResponse? (state.nodes destination) response with
              | none => none
              | some nextNode =>
                  some
                    {
        ...
        | .requestPreVote request =>
            match handleRequestPreVote? (state.nodes destination) request with
            | none => none
            | some (nextNode, response) =>
                some
                  {
        ...
        | .requestPreVoteResponse response =>
            if state.allocated response.source then
              match handleRequestPreVoteResponse? (state.nodes destination) response with
              | none => none
              | some nextNode =>
                  some
                    {
...
def acknowledgingNodes (state : State Node TxId) (leader : Node) (index : Nat)
    : Finset Node :=
...
-/
def hasMajorityAt (state : State Node TxId) (leader : Node) (index : Nat) : Prop :=
...
def hasElectionMajority (state : State Node TxId) (candidate : Node) : Prop :=
...
def hasPreVoteMajority (state : State Node TxId) (candidate : Node) : Prop :=
  (activeConfigurations (state.nodes candidate)).all
    fun configuration =>
      decide
...
def hasOtherActiveReplica (state : State Node TxId) (node : Node) : Prop :=
...
def pendingRetiredCommittedNodes (state : State Node TxId) (leader : Node)
    : Finset Node :=
  state.retirementCompleted leader \ allRetiredCommittedNodes (state.nodes leader).log
...
-/
def highestCommittableIndex (state : State Node TxId) (leader : Node) : Nat :=
...
def terminalRetirementCommit (state : State Node TxId) (node : Node) : Prop :=
...
@[simp]
def becomeCandidateState (state : State Node TxId) (node : Node) : State Node TxId :=
  let nodeState := state.nodes node
  {
    state with
      nodes :=
...
def advanceCommitState (state : State Node TxId) (node : Node) : State Node TxId :=
  let nodeState := state.nodes node
  let refreshed :=
    refreshRetirementState node
...
def stepDownState (state : State Node TxId) (node : Node) : State Node TxId :=
  let nodeState := state.nodes node
  {
    state with
      nodes :=
...
-/
def demoteRetiredCommitted (state : State Node TxId) (node : Node) : State Node TxId :=
  let nodeState := state.nodes node
...
@[reducible]
def definition (state : State Node TxId) : Action Node TxId -> ActionDefinition Node TxId
      ...
      let sourceState := state.nodes source
      {
        enabled :=
          ...
          /\ ((messageEntries sourceState.log
                (sourceState.sentIndex destination) batchEnd).all
                fun entry =>
                  entry.term == termAt sourceState.log batchEnd)
              = true
          /\ (Not (sourceState.membershipState = .retiredCommitted)
              \/ sourceState.sentIndex destination < batchEnd)
        next :=
...
def runActions (state : State Node TxId)
