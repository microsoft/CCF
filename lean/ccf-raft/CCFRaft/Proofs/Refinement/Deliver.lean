...
theorem updateNode_self {nodes : NodeStore Node TxId} {node : Node}
    (allocated : nodes.allocated node) : updateNode nodes node (nodes node) = nodes := by
...
theorem updateQueue_updateQueue (network : Node -> List (Abstract.Model.Message Node TxId))
    (destination : Node) (first second : List (Abstract.Model.Message Node TxId)) :
...
theorem Corr.deliver {concrete : Model.State Node TxId}
    {abstract after : Abstract.Model.State Node TxId} {envelope : Model.Envelope Node TxId}
    {value : NodeState Node TxId} {sends : List (Model.Envelope Node TxId)}
    (corr : Corr concrete abstract)
    (nodesEq : forall member,
      after.nodes member = if member = envelope.target then value else abstract.nodes member)
    (networkEq : forall target,
      after.network target =
        (if target = envelope.target then (abstract.network target).erase (toAbstract envelope)
          else abstract.network target) ++ absQueue sends target)
    ...
    (retirementHere : after.retirementCompleted envelope.target =
      retirementCompletedNodes value.log value.commitIndex)
    (retirementOther : forall member, Not (member = envelope.target) ->
      after.retirementCompleted member = abstract.retirementCompleted member)
    (sendsAllocated : forall sent, sent ∈ sends ->
      after.allocated sent.source /\ after.allocated sent.target) :
    Corr { concrete with
      nodes := replaceNode concrete.nodes envelope.target value
      network := removeOne envelope concrete.network ++ sends } after := by
...
def reordered (abstract : Abstract.Model.State Node TxId) (envelope : Model.Envelope Node TxId) :
    Abstract.Model.State Node TxId :=
  { abstract with
    network := updateQueue abstract.network envelope.target
      (toAbstract envelope :: (abstract.network envelope.target).erase (toAbstract envelope)) }
...
def observed (state : Abstract.Model.State Node TxId) (envelope : Model.Envelope Node TxId) :
    Abstract.Model.State Node TxId :=
  { state with
...
theorem observeTerm_eq_updateTerm (state : NodeState Node TxId)
    ...
    Model.Local.observeTerm state message = state \/
      (Model.Local.observeTerm state message = Model.Local.updateTerm state message.term
        /\ state.currentTerm < message.term
        /\ (forall term, Not (message = .proposeVoteRequest term))
        /\ (forall response, message = .appendEntriesResponse response -> state.role = .leader)) := by
...
theorem moves_observed {abstract : Abstract.Model.State Node TxId}
    ...
    (queued : abstract.network envelope.target = toAbstract envelope ::
      (abstract.network envelope.target).tail)
...
theorem receive_appendRequest_stepDown {request : Model.Local.AppendEntriesRequest Node TxId}
    (queued : state.network destination =
      .appendEntriesRequest (absAppendRequest request source destination) :: rest)
    (stepping : request.term = (state.nodes destination).currentTerm /\
    ...
    Abstract.Model.handleReceive? state source destination =
      some { state with
        nodes := updateNode state.nodes destination (stepDown (state.nodes destination) request) } := by
...
theorem receive_appendRequest {request : Model.Local.AppendEntriesRequest Node TxId}
    (queued : state.network destination =
      .appendEntriesRequest (absAppendRequest request source destination) :: rest)
    (noStepDown : Not (request.term = (state.nodes destination).currentTerm /\
      ((state.nodes destination).role = .candidate \/
        (state.nodes destination).role = .preVoteCandidate)))
    {next : NodeState Node TxId} {response : Model.Local.AppendEntriesResponse}
    (handled : Model.Local.handleAppendEntriesRequest? destination (state.nodes destination) request =
      some (next, response)) :
    Abstract.Model.handleReceive? state source destination =
      some { state with
        nodes := updateNode state.nodes destination (refreshRetirementState destination next)
        network := Abstract.Model.reply state.network destination rest
          (absAppendResponse response destination source)
        retirementCompleted := Abstract.Model.refreshRetirementCompleted state.retirementCompleted
          destination (refreshRetirementState destination next) } := by
  unfold Abstract.Model.handleReceive?
  rw [queued, takeFirstFrom_head (source := source) (rest := rest) _ rfl]
  simp only [Abstract.Model.Message.destination, absAppendRequest_destination, bne_self_eq_false,
...
theorem receive_appendResponse {response : Model.Local.AppendEntriesResponse}
    ...
    (bounded : (state.nodes destination).role = .leader ->
      response.term <= (state.nodes destination).currentTerm) :
    Abstract.Model.handleReceive? state source destination =
      some { state with
        nodes := updateNode state.nodes destination
          (Model.Local.handleAppendEntriesResponse (state.nodes destination) source response)
        network := updateQueue state.network destination rest } := by
  unfold Abstract.Model.handleReceive?
  rw [queued, takeFirstFrom_head (source := source) (rest := rest) _ rfl]
...
theorem receive_voteRequest {request : Model.Local.RequestVoteRequest}
    ...
    Abstract.Model.handleReceive? state source destination =
      some { state with
        nodes := updateNode state.nodes destination
          (Model.Local.handleRequestVoteRequest (state.nodes destination) source request).1
        network := enqueue (updateQueue state.network destination rest)
          (.requestVoteResponse (absVoteResponse
            (Model.Local.handleRequestVoteRequest (state.nodes destination) source request).2
...
theorem receive_voteResponse {response : Model.Local.RequestVoteResponse}
    ...
    Abstract.Model.handleReceive? state source destination =
      some { state with
        nodes := updateNode state.nodes destination
          (Model.Local.handleRequestVoteResponse (state.nodes destination) source response)
        network := updateQueue state.network destination rest } := by
  unfold Abstract.Model.handleReceive?
  rw [queued, takeFirstFrom_head (source := source) (rest := rest) _ rfl]
...
theorem receive_preVote {request : Model.Local.RequestVoteRequest}
    ...
    Abstract.Model.handleReceive? state source destination =
      some { state with
        ...
        network := enqueue (updateQueue state.network destination rest)
          (.requestPreVoteResponse (absPreVoteResponse
...
theorem receive_preVoteResponse {response : Model.Local.RequestVoteResponse}
    (queued : state.network destination =
      .requestPreVoteResponse (absPreVoteResponse response source destination) :: rest)
    (sourceAllocated : state.allocated source)
    (bounded : response.term <= (state.nodes destination).currentTerm) :
    Abstract.Model.handleReceive? state source destination =
      some { state with
        nodes := updateNode state.nodes destination
          (Model.Local.handleRequestPreVoteResponse (state.nodes destination) source response)
        network := updateQueue state.network destination rest } := by
  unfold Abstract.Model.handleReceive?
  rw [queued, takeFirstFrom_head (source := source) (rest := rest) _ rfl]
      ...
      some { state with
        nodes := updateNode state.nodes destination
          (if term = (state.nodes destination).currentTerm /\
            ...
            Model.Local.becomeCandidateNodeState (state.nodes destination) destination
...
structure Staged (abstract pre : Abstract.Model.State Node TxId)
    (envelope : Model.Envelope Node TxId) (receiver : NodeState Node TxId) : Prop where
  nodes : forall member,
    pre.nodes member = if member = envelope.target then receiver else abstract.nodes member
  network : pre.network = updateQueue abstract.network envelope.target
    (toAbstract envelope :: (abstract.network envelope.target).erase (toAbstract envelope))
...
theorem Staged.queue {abstract pre : Abstract.Model.State Node TxId}
    ...
    pre.network envelope.target =
      toAbstract envelope :: (abstract.network envelope.target).erase (toAbstract envelope) := by
  rw [staged.network]
  simp [updateQueue]

theorem Staged.receiver {abstract pre : Abstract.Model.State Node TxId}
    {envelope : Model.Envelope Node TxId} {receiver : NodeState Node TxId}
    (staged : Staged abstract pre envelope receiver) : pre.nodes envelope.target = receiver := by
  simp [staged.nodes]
...
theorem prepare {concrete : Model.State Node TxId} {abstract : Abstract.Model.State Node TxId}
    (corr : Corr concrete abstract) {envelope : Model.Envelope Node TxId}
    (member : envelope ∈ concrete.network) :
    Moves abstract (observed (reordered abstract envelope) envelope) /\
      Staged abstract (observed (reordered abstract envelope) envelope) envelope
        (Model.Local.observeTerm (abstract.nodes envelope.target) envelope.payload) := by
...
theorem stage_stepDown {abstract pre : Abstract.Model.State Node TxId}
    ...
    (staged : Staged abstract pre ⟨source, destination, .appendEntriesRequest request⟩ receiver)
    (targetAllocated : abstract.allocated destination)
    (stepping : request.term = receiver.currentTerm /\
      (receiver.role = .candidate \/ receiver.role = .preVoteCandidate)) :
    Moves pre { pre with nodes := updateNode pre.nodes destination (stepDown receiver request) } /\
      Staged abstract { pre with nodes := updateNode pre.nodes destination (stepDown receiver request) }
        ⟨source, destination, .appendEntriesRequest request⟩ (stepDown receiver request) := by
...
theorem finish {concrete : Model.State Node TxId} {abstract pre post : Abstract.Model.State Node TxId}
    (corr : Corr concrete abstract) {envelope : Model.Envelope Node TxId}
    (member : envelope ∈ concrete.network) {receiver value : NodeState Node TxId}
    {sends : List (Model.Envelope Node TxId)}
    (staged : Staged abstract pre envelope receiver)
    (received : Abstract.Model.handleReceive? pre envelope.source envelope.target = some post)
    (nodesPost : post.nodes = updateNode pre.nodes envelope.target value)
    (networkPost : forall target,
      post.network target =
    ...
    (retirementHere : post.retirementCompleted envelope.target =
      retirementCompletedNodes value.log value.commitIndex)
    (retirementOther : forall member, Not (member = envelope.target) ->
      post.retirementCompleted member = pre.retirementCompleted member)
    (sendsEndpoints : forall sent, sent ∈ sends ->
      sent.source = envelope.target /\ sent.target = envelope.source) :
    exists after, Moves pre after /\
      Corr { concrete with
        nodes := replaceNode concrete.nodes envelope.target value
        network := removeOne envelope concrete.network ++ sends } after := by
  obtain ⟨sourceAllocated, targetAllocated⟩ := corr.endpoints envelope member
...
theorem observeTerm_log (state : NodeState Node TxId) (message : Model.Local.Message Node TxId) :
...
theorem simulate_appendEntriesRequest (corr : Corr concrete abstract)
    ...
    (staged : Staged abstract pre ⟨source, destination, .appendEntriesRequest request⟩ receiver)
    {next : NodeState Node TxId} {response : Model.Local.AppendEntriesResponse}
    (handled : Model.Local.handleAppendEntriesRequest? destination receiver request =
      some (next, response)) :
    exists after, Moves pre after /\
      Corr { concrete with
...
theorem simulate_plainResponse {pre post : Abstract.Model.State Node TxId}
    ...
    (staged : Staged abstract pre envelope receiver)
    (received : Abstract.Model.handleReceive? pre envelope.source envelope.target = some post)
    (postEq : post = { pre with
      nodes := updateNode pre.nodes envelope.target value
      network := updateQueue pre.network envelope.target
    ...
    exists after, Moves pre after /\
      Corr { concrete with
        nodes := replaceNode concrete.nodes envelope.target value
        network := removeOne envelope concrete.network ++ [] } after := by
  subst postEq
...
theorem simulate_withResponse {pre post : Abstract.Model.State Node TxId}
    ...
    (postEq : post = { pre with
      nodes := updateNode pre.nodes envelope.target value
      network := enqueue (updateQueue pre.network envelope.target
        ((abstract.network envelope.target).erase (toAbstract envelope))) (toAbstract reply) })
    (logEq : value.log = (abstract.nodes envelope.target).log)
    (commitEq : value.commitIndex = (abstract.nodes envelope.target).commitIndex)
    (replySource : reply.source = envelope.target) (replyTarget : reply.target = envelope.source) :
    exists after, Moves pre after /\
      Corr { concrete with
        nodes := replaceNode concrete.nodes envelope.target value
        network := removeOne envelope concrete.network ++ [reply] } after := by
  subst postEq
...
theorem handleProposeVoteRequest_log (state : NodeState Node TxId) (self : Node) (term : Nat) :
    (Model.Local.handleProposeVoteRequest state self term).log = state.log /\
      (Model.Local.handleProposeVoteRequest state self term).commitIndex = state.commitIndex := by
  unfold Model.Local.handleProposeVoteRequest
  split_ifs <;> exact ⟨rfl, rfl⟩

/-- Every enabled delivery is simulated by abstract moves. -/
theorem simulate_deliver (corr : Corr concrete abstract) {envelope : Model.Envelope Node TxId}
    (member : envelope ∈ concrete.network) {state : NodeState Node TxId}
    ...
    exists after, Moves abstract after /\
      Corr { concrete with
        ...
        network := removeOne envelope concrete.network ++ (execute.run {}).2.outgoing } after := by
