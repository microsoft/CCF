...
open Model.Local (
  NodeState Bootstrap Role Entry INITIAL_CONFIGURATION INITIAL_LEADER
...
open Abstract.Invariant (
  SystemInductiveInvariant JoinedCarrierFacts AllocatedNodesExactlyJoined)
...
theorem absQueue_append (first second : List (Model.Envelope Node TxId)) (target : Node)
    : absQueue (first ++ second) target
      = absQueue first target ++ absQueue second target := by
  simp [absQueue]

theorem absQueue_nil (target : Node)
    : absQueue ([] : List (Model.Envelope Node TxId)) target = [] :=
  rfl
...
theorem absQueue_erase
    ...
    : absQueue (network.erase envelope) target
      = if envelope.target = target then
          (absQueue network target).erase (toAbstract envelope)
        else
          absQueue network target := by
  unfold absQueue
...
theorem joinedCarriers {abstract : Abstract.Model.State Node TxId}
    (invariant : SystemInductiveInvariant abstract)
    : JoinedCarrierFacts abstract := by
...
theorem allocated_of_role {abstract : Abstract.Model.State Node TxId} {node : Node}
    (role : Not ((abstract.nodes node).role = .none))
    : abstract.allocated node := by
  by_contra absent
...
theorem allocated_of_activeNodeUnion {abstract : Abstract.Model.State Node TxId}
    ...
    (active : member ∈ activeNodeUnion (abstract.nodes node))
    : abstract.allocated member :=
  (allocatedJoined invariant member).mpr
    ((joinedCarriers invariant).activeNodes node active)
...
theorem Corr.update {concrete : Model.State Node TxId}
    ...
    (retirement
      : forall member,
          ...
          = retirementCompletedNodes (after.nodes member).log
              (after.nodes member).commitIndex)
    (endpoints
      : forall envelope,
          envelope ∈ network
          -> after.allocated envelope.source /\ after.allocated envelope.target)
    : Corr { concrete with nodes := replaceNode concrete.nodes node value, network }
        after where
