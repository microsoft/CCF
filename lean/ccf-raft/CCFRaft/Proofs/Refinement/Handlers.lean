...
def absAppendRequest (request : Model.Local.AppendEntriesRequest Node TxId) (source destination : Node) :
...
def absAppendResponse (response : Model.Local.AppendEntriesResponse) (source destination : Node) :
...
def absVoteResponse (response : Model.Local.RequestVoteResponse) (source destination : Node) :
...
def absPreVoteResponse (response : Model.Local.RequestVoteResponse) (source destination : Node) :
...
@[simp] theorem absAppendResponse_source (response : Model.Local.AppendEntriesResponse) (source destination : Node) :
    (absAppendResponse response source destination).source = source := rfl
@[simp] theorem absAppendResponse_destination (response : Model.Local.AppendEntriesResponse) (source destination : Node) :
    (absAppendResponse response source destination).destination = destination := rfl
@[simp] theorem absVoteRequest_source (request : Model.Local.RequestVoteRequest) (source destination : Node) :
    (absVoteRequest request source destination).source = source := rfl
@[simp] theorem absVoteRequest_destination (request : Model.Local.RequestVoteRequest) (source destination : Node) :
    (absVoteRequest request source destination).destination = destination := rfl
@[simp] theorem absVoteResponse_source (response : Model.Local.RequestVoteResponse) (source destination : Node) :
    (absVoteResponse response source destination).source = source := rfl
@[simp] theorem absVoteResponse_destination (response : Model.Local.RequestVoteResponse) (source destination : Node) :
    (absVoteResponse response source destination).destination = destination := rfl
@[simp] theorem absPreVote_source (request : Model.Local.RequestVoteRequest) (source destination : Node) :
    (absPreVote request source destination).source = source := rfl
@[simp] theorem absPreVote_destination (request : Model.Local.RequestVoteRequest) (source destination : Node) :
    (absPreVote request source destination).destination = destination := rfl
@[simp] theorem absPreVoteResponse_source (response : Model.Local.RequestVoteResponse) (source destination : Node) :
    (absPreVoteResponse response source destination).source = source := rfl
@[simp] theorem absPreVoteResponse_destination (response : Model.Local.RequestVoteResponse) (source destination : Node) :
    (absPreVoteResponse response source destination).destination = destination := rfl

variable [Bootstrap Node]

section AppendEntries

variable (state : NodeState Node TxId) (request : Model.Local.AppendEntriesRequest Node TxId)
...
theorem failureResponse_eq :
    Abstract.Model.failureResponse state (absAppendRequest request source destination) =
      absAppendResponse (Model.Local.failureResponse state request) destination source := by
...
@[simp] theorem absAppendRequest_term :
    (absAppendRequest request source destination).term = request.term := rfl
@[simp] theorem absAppendRequest_prevLogIndex :
    (absAppendRequest request source destination).prevLogIndex = request.prevLogIndex := rfl
@[simp] theorem absAppendRequest_prevLogTerm :
    (absAppendRequest request source destination).prevLogTerm = request.prevLogTerm := rfl
@[simp] theorem absAppendRequest_entries :
    (absAppendRequest request source destination).entries = request.entries := rfl
@[simp] theorem absAppendRequest_leaderCommit :
    (absAppendRequest request source destination).leaderCommit = request.leaderCommit := rfl
@[simp] theorem absAppendRequest_source :
    (absAppendRequest request source destination).source = source := rfl
@[simp] theorem absAppendRequest_destination :
    (absAppendRequest request source destination).destination = destination := rfl

theorem logOk_iff :
    Abstract.Model.logOk state (absAppendRequest request source destination) <->
      Model.Local.logOk state request := Iff.rfl

theorem alreadyDone_iff :
    Abstract.Model.alreadyDone state (absAppendRequest request source destination) <->
      Model.Local.alreadyDone state request := Iff.rfl

theorem hasTermConflict_iff :
    Abstract.Model.hasTermConflict state (absAppendRequest request source destination) <->
      Model.Local.hasTermConflict state request := Iff.rfl

theorem noConflictExtension_iff :
    Abstract.Model.noConflictExtension state (absAppendRequest request source destination) <->
      Model.Local.noConflictExtension state request := Iff.rfl

theorem committedFromLeader_eq (log : List (Model.Local.Entry Node TxId)) :
    Abstract.Model.committedFromLeader state (absAppendRequest request source destination) log =
      Model.Local.committedFromLeader state request log := rfl

theorem reject_eq :
    Abstract.Model.rejectAppendEntriesRequest? state (absAppendRequest request source destination) =
      (Model.Local.rejectAppendEntriesRequest? state request).map (absResult source destination) := by
...
theorem alreadyDone_eq :
    Abstract.Model.appendEntriesAlreadyDone? state (absAppendRequest request source destination) =
      (Model.Local.appendEntriesAlreadyDone? state request).map (absResult source destination) := by
  unfold Abstract.Model.appendEntriesAlreadyDone? Model.Local.appendEntriesAlreadyDone?
  simp only [alreadyDone_iff]
  split_ifs <;> rfl

theorem conflict_eq :
    Abstract.Model.conflictAppendEntriesRequest? state (absAppendRequest request source destination) =
...
theorem noConflict_eq :
    Abstract.Model.noConflictAppendEntriesRequest? state (absAppendRequest request source destination) =
...
theorem accept_eq :
    Abstract.Model.acceptAppendEntriesRequest? state (absAppendRequest request source destination) =
...
theorem handleAppendEntriesRequest_eq
    (noStepDown : Not (request.term = state.currentTerm /\
      (state.role = .candidate \/ state.role = .preVoteCandidate))) :
    Abstract.Model.handleAppendEntriesRequest? state (absAppendRequest request source destination) =
...
def stepDown (state : NodeState Node TxId) (request : Model.Local.AppendEntriesRequest Node TxId) :
    NodeState Node TxId :=
...
theorem stepDown_noStepDown (state : NodeState Node TxId)
    ...
    Not (request.term = (stepDown state request).currentTerm /\
...
theorem handleAppendEntriesRequest_stepDown (self : Node) (state : NodeState Node TxId)
    ...
    Model.Local.handleAppendEntriesRequest? self state request =
      Model.Local.handleAppendEntriesRequest? self (stepDown state request) request := by
...
theorem returnToFollower_eq (state : NodeState Node TxId)
    ...
    Abstract.Model.returnToFollowerState? state (absAppendRequest request source destination) =
      if request.term = state.currentTerm /\
          (state.role = .candidate \/ state.role = .preVoteCandidate) then
...
theorem handleAppendEntriesResponse_eq (state : NodeState Node TxId)
    ...
    Abstract.Model.handleAppendEntriesResponse? state (absAppendResponse response source destination) =
...
theorem voteLogUpToDate_iff (state : NodeState Node TxId) (request : Model.Local.RequestVoteRequest)
    (source destination : Node) :
    Abstract.Model.voteLogUpToDate state (absVoteRequest request source destination) <->
      Model.Local.voteLogUpToDate state request := Iff.rfl
...
theorem handleRequestVoteRequest_eq (state : NodeState Node TxId)
    ...
    Abstract.Model.handleRequestVoteRequest? state (absVoteRequest request source destination) =
      some ((Model.Local.handleRequestVoteRequest state source request).1,
...
theorem handleRequestPreVote_eq (state : NodeState Node TxId)
    ...
    Abstract.Model.handleRequestPreVote? state (absPreVote request source destination) =
      some (state, absPreVoteResponse (Model.Local.handleRequestPreVote state request)
        destination source) := by
...
theorem handleRequestVoteResponse_eq (state : NodeState Node TxId)
    ...
    Abstract.Model.handleRequestVoteResponse? state (absVoteResponse response source destination) =
