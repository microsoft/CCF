...
inductive LegacyAtomic :
    DisasterRecoveryMigration.Legacy.Phase -> DisasterRecoveryMigration.Legacy.Phase -> Prop where
...
theorem initial_phase_correspondence :
    projectPhase (initialNode "node0") = DisasterRecoveryMigration.Legacy.Phase.vote := by
  rfl
...
theorem three_node_initial_correspondence :
    ...
    ((initialSystem config).nodes.map
      ...
      (DisasterRecoveryMigration.Legacy.initialState 3).actors.toList.map
...
theorem single_node_full_initial_models_differ :
    projectPhase (initialNode "0") !=
      (DisasterRecoveryMigration.Legacy.initialState 1).actors[0]!.nextStep := by
  decide

end DisasterRecoveryMigration.Refinement