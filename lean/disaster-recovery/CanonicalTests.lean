...
def main : IO UInt32 := do
  ...
  let first :=
    step config initial (.receiveGossip "A" { view := 1, seqno := 10 } .accepted)
  expect (first.state.phase == .gossiping) "one of two gossips advanced early"
  let duplicate :=
    step config first.state (.receiveGossip "A" { view := 99, seqno := 99 } .accepted)
  expect (duplicate.state == first.state)
    "duplicate gossip source changed its recorded TxID"
  let second :=
    step config first.state (.receiveGossip "B" { view := 2, seqno := 1 } .accepted)
  ...
  let tiedA :=
    step config initial (.receiveGossip "A" { view := 2, seqno := 1 } .accepted)
  let tiedB :=
    step config tiedA.state (.receiveGossip "B" { view := 2, seqno := 1 } .accepted)
  expect (tiedB.state.chosen == some "B")
    "location name did not break an equal TxID tie lexicographically"

  let frozen :=
    step config second.state (.receiveGossip "C" { view := 9, seqno := 9 } .accepted)
  ...
  expect (noVotes.state == emptyVoting) "aligned voting timeout with zero votes advanced"
  ...
  let joining := step config initial (.receiveIAmOpen "B" .accepted)
  ...
  expect
    (retry.effects == [.sendVote "B", .sendGossip "A", .sendGossip "B"])
  ...
  let unexpected :=
    step unexpectedConfig (initialNode "A")
