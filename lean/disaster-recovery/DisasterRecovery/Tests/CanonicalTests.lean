...
example (config : Model.Config) (active : List Location)
    (valid : config.Valid)
    (nodup : active.Nodup)
    (configured : forall node, node ∈ active -> node ∈ config.protocol.expectedLocations)
...
private def runLocal (config : Config) (state : NodeState) (event : Event)
  ...
  requireSome
    ((step (Capabilities.record state.location) config { view := 1, seqno := 10 } state
        event).map
...
private def runGlobal (config : Model.Config) (state : Model.State)
    (action : Model.Action)
    : IO Model.State :=
  requireSome ((Model.transitionSystem config).step state action)
    s!"disabled global action: {repr action}"

private def deliverMessage (config : Model.Config) (state : Model.State)
    (source target : Location) (payload : Message)
    : IO Model.State := do
...
private def eventsFor (config : Config) : List Event :=
  let messages :=
    config.expectedLocations.flatMap
      fun source =>
        [
...
private def invariant (state : NodeState) : Bool :=
  let chosenReady := if state.phase == .voting then state.chosen.isSome else true
  let openingKind :=
        ...
        expect
          (afterPending == after
            ...
            && accumulated.notifications
                == pending.notifications ++ effects.notifications)
...
def main : IO UInt32 := do
  ...
  for event
      in [
        ...
        .receiveIAmOpen "B" .rejected
      ] do
    let (after, effects) <- runLocal config second.1 event
    expect
      (after == second.1
        && effects.outgoing.isEmpty
  ...
  expect
    (machine.step misaligned .timeout
      == some { misaligned with timeoutState := .opening })
  ...
  for (before, expected) in retryCases do
    expect
      (retry globalConfig before == expected.map (fun sent => (before, sent)))
      s!"direct retry sends differ in {repr before.phase}"
  let noPeers := { globalConfig with protocol := { config with expectedLocations := [] } }
  expect ((retry noPeers initial).isNone) "empty gossip retry must be disabled"
  expect
  ...
  expect
    (((Model.transitionSystem globalConfig).step globalInitial
        (.local "A" .timeout)).isNone)
    "global empty-gossip timeout must remain disabled"
  expect
    (((Model.transitionSystem globalConfig).step globalInitial
        (.local "C" .retry)).isNone)
  ...
  let message <- requireSome (sent.network.find? fun envelope => envelope.target == "A")
                  "self-gossip missing"
  ...
  expect
    (MultiNodeTransitionSystem.nodeState delivered "B"
      == MultiNodeTransitionSystem.nodeState sent "B")
  ...
  let inactiveMessage <- requireSome
                          (inactiveSent.network.find? fun env => env.target == "B")
                          "inactive gossip missing"
  expect
    (((Model.transitionSystem globalConfig).step inactiveSent
        (.deliver inactiveMessage)).isNone)
  ...
  let opener <- requireSome (MultiNodeTransitionSystem.nodeState receivedBoth "B")
                  "opener disappeared"
  ...
  let joiner <- requireSome (MultiNodeTransitionSystem.nodeState joined "A")
                  "joiner disappeared"
  expect
    (joiner.phase == .joining && joiner.chosen == some "B" && joiner.restartRequested)
  ...
  let opened <- requireSome (MultiNodeTransitionSystem.nodeState timeout3 "B")
                  "opened node disappeared"
  expect (opened.phase == .open) "global timeout lane did not complete opening"
  expect
    (((Model.transitionSystem globalConfig).step timeout3 (.local "B" .retry)).isNone)
