...
lemma step_votes_shape (config : Model.Config) (state : NodeState) (event : Event)
...
lemma recordEffect_preserves_openings_valid
    {config : Config}
    {node : Location}
    {nodeState : NodeState}
    {state : State}
...
lemma recordEffects_preserves_openings_valid
    {config : Config}
    {node : Location}
    {nodeState : NodeState}
    {state : State}
...
lemma next_preserves_node_votes_nodup
    {config : Config}
    {before after : State}
    {action : Action}
...
lemma next_preserves_voting_selections
    {config : Config}
    {before after : State}
    {action : Action}
...
lemma retry_preserves_sent_votes_selected
    {config : Config}
    {before after : State}
    {source : Location}
...
lemma deliver_preserves_sent_votes_selected
    {config : Config}
    {before after : State}
    {envelope : Envelope}
    (selected : SentVotesSelected before)
...
lemma timeout_preserves_sent_votes_selected
    {config : Config}
    {before after : State}
    {target : Location}
    (selected : SentVotesSelected before)
...
lemma retry_preserves_node_votes_sent
    {config : Config}
    {before after : State}
    {source : Location}
...
lemma deliver_preserves_node_votes_sent
    {config : Config}
    {before after : State}
    {envelope : Envelope}
    (wellFormed : WellFormed config before)
...
lemma timeout_preserves_node_votes_sent
    {config : Config}
    {before after : State}
    {target : Location}
    (votesSent : NodeVotesSent before)
...
lemma retry_preserves_openings_valid
    {config : Config}
    {before after : State}
    {source : Location}
...
lemma deliver_preserves_openings_valid
    {config : Config}
    {before after : State}
    {envelope : Envelope}
    (wellFormed : WellFormed config before)
...
lemma timeout_preserves_openings_valid
    {config : Config}
    {before after : State}
    {target : Location}
    (wellFormed : WellFormed config before)
...
lemma retry_preserves_sent_vote_stable
    {config : Config}
    {before after : State}
    {source : Location}
    (wellFormed : WellFormed config before)
...
lemma deliver_preserves_sent_vote_stable
    {config : Config}
    {before after : State}
    {delivered : Envelope}
    (stable : SentVoteStable before)
...
lemma timeout_preserves_sent_vote_stable
    {config : Config}
    {before after : State}
    {target : Location}
    (stable : SentVoteStable before)
...
lemma retry_preserves_sent_votes_functional
    {config : Config}
    {before after : State}
    {source : Location}
    (wellFormed : WellFormed config before)
...
lemma timeout_preserves_sent_votes_functional
    {config : Config}
    {before after : State}
    {target : Location}
    (functional : SentVotesFunctional before)
