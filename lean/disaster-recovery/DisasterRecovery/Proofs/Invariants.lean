...
lemma valid_gossip_uses_recovered_txid
    {config : Config}
    {envelope : Envelope}
    {txid : TxID}
    (valid : envelope.Valid config)
...
lemma recordEffect_preserves_histories_active
    {node : Location}
    {nodeState : NodeState}
    {state : State}
    {effect : Effect}
...
lemma recordEffects_preserves_histories_active
    {node : Location}
    {nodeState : NodeState}
    {effects : List Effect}
    {state : State}
...
lemma mem_openings_recordEffect
    {node : Location}
    {nodeState : NodeState}
    {state : State}
    {effect : Effect}
...
lemma mem_restarts_recordEffects
    {node : Location}
    {nodeState : NodeState}
    {state : State}
    {effects : List Effect}
...
lemma mem_completed_recordEffects
    {node : Location}
    {nodeState : NodeState}
    {state : State}
    {effects : List Effect}
...
lemma next_completed_monotonic
    {config : Config}
    {before after : State}
    {action : Action}
    (transition : next config before action = some after) :
...
lemma retry_preserves_well_formed
    {config : Config}
    {before after : State}
    {source : Location}
...
lemma deliver_preserves_well_formed
    {config : Config}
    {before after : State}
    {envelope : Envelope}
    (wellFormed : WellFormed config before)
...
lemma timeout_preserves_well_formed
    {config : Config}
    {before after : State}
    {target : Location}
    (wellFormed : WellFormed config before)
...
lemma next_preserves_well_formed
    {config : Config}
    {before after : State}
    {action : Action}
    (wellFormed : WellFormed config before)
