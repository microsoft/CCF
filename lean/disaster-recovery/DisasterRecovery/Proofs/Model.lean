...
lemma valid_timeout_requires_alignment
    ...
    : state.phase = state.timeoutState := by simpa [validTimeout] using h
...
lemma opening_rejects_iamopen (config : Config) (state : NodeState) (source : Location)
...
lemma open_rejects_iamopen (config : Config) (state : NodeState) (source : Location)
...
lemma aligned_voting_timeout_without_votes_stutters (config : Config) (state : NodeState)
    : let waiting :=
        {
          state with
            phase := .voting
...
lemma aligned_empty_gossip_timeout_aborts (config : Config) (state : NodeState)
    : let waiting :=
        {
          state with
            phase := .gossiping
