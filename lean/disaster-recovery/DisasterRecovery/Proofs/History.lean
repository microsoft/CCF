...
lemma trace_lifts {config : Model.Config} {before after : Model.State} {tail}
    ...
    exists final, Decoration config ghost tail final /\
  ...
  | @cons before middle after tail action step rest ih =>
      ...
      exact ⟨final, middleEq ▸ Decoration.cons ghostStep decorated, reachableFinal, finalEq⟩

structure Correspondence (config : Model.Config) (trace : Properties.GlobalTrace)
    (ghost : Execution.Global.State) : Prop where
  reachable : Execution.Global.Reachable config ghost
  suffix : forall state, state ∈ trace.states ->
    exists tail, Trace.Path (Model.transitionSystem config) state tail (erase ghost)
...
theorem history_correspondence {config : Model.Config} (trace : Properties.GlobalTrace)
    ...
    exists initial tail final,
      ...
      Decoration config initial tail final /\ Correspondence config trace final := by
  ...
  refine ⟨initial, tail, final, by simpa only [linked] using states,
    init, decorated, reachable, ?_⟩
  intro state member
  rw [states] at member
