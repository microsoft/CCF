...
lemma node_property_of_nodeState
    {state : State}
    {node : Location}
    {foundState : NodeState}
...
lemma completed_effect_recorded
    {node : Location}
    {nodeState : NodeState}
    {effects : List Effect}
    {state : State}
...
lemma recovered_for_configured
    {config : Config}
    (valid : config.Valid)
    {node : Location}
...
lemma retryMessages_self_gossip
    {config : Config}
    {node : Location}
    {state : NodeState}
    {txid : TxID}
...
lemma timeout_enabled_of_accepted
    {config : Config}
    {state : State}
    {node : Location}
    {nodeState : NodeState}
...
lemma next_preserves_advanced_lane
    {config : Config}
    {before after : State}
    {action : Action}
    {node : Location}
...
lemma opening_progress_between
    {config : Config}
    (execution : Execution config)
    (initial : Reachable config (execution.states 0))
    {start finish : Nat}
...
lemma deliver_iamopen_resolves
    {config : Config}
    {before after : State}
    {envelope : Envelope}
    (wellFormed : WellFormed config before)
...
lemma aligned_timeout_voting_opens
    {config : Config}
    {before after : State}
    {node : Location}
    {nodeState : NodeState}
...
lemma next_gossiping_predecessor
    {config : Config}
    {before after : State}
    {action : Action}
    {node : Location}
...
lemma reachable_announcements_live
    {config : Config}
    {state : State}
    (reachable : Reachable config state) :
...
lemma reachable_joining_announcements
    {config : Config}
    {state : State}
    (reachable : Reachable config state) :
...
lemma reachable_open_completed
    {config : Config}
    {state : State}
    (reachable : Reachable config state) :
...
lemma reachable_announcements_resolved
    {config : Config}
    {state : State}
    (reachable : Reachable config state) :
...
lemma joining_node_announcement
    {state : State}
    {node : Location}
    {nodeState : NodeState}
...
lemma openerWitness_of_later_phase
    {config : Config}
    {state : State}
    {node : Location}
    (reachable : Reachable config state)
...
lemma openerWitness_after_leave_voting
    {config : Config}
    (execution : Execution config)
    (initial : Reachable config (execution.states 0))
    {start later : Nat}
...
lemma reachable_advanced_active
    {config : Config}
    {state : State}
    (reachable : Reachable config state) :
...
lemma openerWitness_eventually_completes
    {config : Config}
    (execution : Execution config)
    (initial : Reachable config (execution.states 0))
    (fair : Fair execution)
...
lemma fair_some_opener_completes
    {config : Config}
    (execution : Execution config)
    (initial : Reachable config (execution.states 0))
    (fair : Fair execution)
...
lemma fair_target_terminal_after_completion
    {config : Config}
    (execution : Execution config)
    (initial : Reachable config (execution.states 0))
    (fair : Fair execution)
...
lemma single_completion_path_joins_others
    {config : Config}
    (execution : Execution config)
    (initial : Reachable config (execution.states 0))
    (fair : Fair execution)
