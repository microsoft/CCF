...
)
{
  ...
  )
  {
    ...
    return uu____1.sz == (uint32_t)1U + mt_path_length((uint32_t)0U, k2, j1, false);
  }
  else
  {
    return false;
...
static bool buf_eq__uint8_t(uint8_t *b1, uint8_t *b2, uint32_t len)
{
  if (len == (uint32_t)0U)
  {
    return true;
  ...
  {
    uint8_t a1 = b1[len - (uint32_t)1U];
    uint8_t a2 = b2[len - (uint32_t)1U];
    bool teq = buf_eq__uint8_t(b1, b2, len - (uint32_t)1U);
    return a1 == a2 && teq;
  }
}
