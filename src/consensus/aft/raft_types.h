...
namespace aft
{
  ...
  public:
    ...
    virtual void rollback(const kv::TxID& tx_id, Term commit_term) = 0;
  ...
  public:
    ...
    void rollback(const kv::TxID& tx_id, Term commit_term) override
    {
      auto p = x.lock();
      if (p)
      {
        p->rollback(tx_id, commit_term);
      }
    }
