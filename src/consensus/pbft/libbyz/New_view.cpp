...
{
  PBFT_ASSERT(get_node()->is_replica(id), "Not a replica");
...
{
  if (id < 0 || id >= get_node()->num_of_replicas())
  {
    return false;
  }
...
bool New_view::pre_verify()
{
  ...
  for (Seqno i = min(); i < max(); i++)
  {
    ...
    if (!get_node()->is_replica(vci) || vc_info()[vci].d.is_zero())
    {
      return false;
    }
  }
  ...
  int old_size = sizeof(New_view_rep) +
    sizeof(VC_info) * get_node()->num_of_replicas() + max() - min();

  if (Max_message_size - old_size < get_node()->sig_size(id()))
  {
    return false;
  }
