...
Checkpoint::Checkpoint(Seqno s, Digest& d, bool stable) :
  ...
  Message(Checkpoint_tag, sizeof(Checkpoint_rep) + get_node()->auth_size())
{
#else
  Message(Checkpoint_tag, sizeof(Checkpoint_rep) + get_node()->sig_size())
{
#endif
...
bool Checkpoint::pre_verify()
{
  ...
  if (size() - (int)sizeof(Checkpoint_rep) < get_node()->auth_size(id()))
  {
    return false;
  }
