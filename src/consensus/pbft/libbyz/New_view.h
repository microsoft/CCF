...
struct New_view_rep : public Message_rep
{
  ...
  /* Followed by:
     ...
     // the new-view.
     VC_info vc_info[get_node()->num_of_replicas()];
...
public:
  ...
  // Requires: id >= 0 && id < get_node()->num_of_replicas())
...
{
  // Effects: Returns a pointer to the picked array.
  return (uint8_t*)(vc_info() + get_node()->num_of_replicas());
}
