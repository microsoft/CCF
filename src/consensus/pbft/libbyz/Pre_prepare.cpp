...
#ifndef USE_PKEY
  char* max_req = next_req + msize() - get_replica()->max_nd_bytes() -
    get_node()->auth_size();
#else
  char* max_req =
    next_req + msize() - get_replica()->max_nd_bytes() - get_node()->sig_size();
#endif
...
#pragma GCC diagnostic pop
    ...
    else
    {
        ...
        next_req + sizeof(Digest) <= max_req)
      {
        ...
        get_replica()->big_reqs()->add_pre_prepare(reqs.remove(), s, v);
  ...
  if (rep().rset_size > 0 || n_big_reqs > 0)
  {
    ...
    int non_det_size = get_replica()->max_nd_bytes();
    get_replica()->compute_non_det(s, non_det_choices(), &non_det_size);
...
{
  return get_replica()->primary(view());
}
...
#ifdef SIGN_BATCH
  if (
    get_replica()->should_sign_next_and_reset() ||
    (rep().seqno == get_replica()->next_expected_sig_offset()) ||
    get_node()->f() == 0)
  {
    get_replica()->set_next_expected_sig_offset();
...
bool Pre_prepare::calculate_digest(Digest& d)
{
  ...
  int min_size = sizeof(Pre_prepare_rep) + rep().rset_size +
    rep().n_big_reqs * sizeof(Digest) + rep().non_det_size +
    get_node()->auth_size(get_replica()->primary(view()));
#else
  int min_size = sizeof(Pre_prepare_rep) + rep().rset_size +
    rep().n_big_reqs * sizeof(Digest) + rep().non_det_size +
    get_node()->sig_size(get_replica()->primary(view()));
#endif
  if (size() >= min_size)
  {
    START_CC(pp_digest_cycles);
...
{
  int sender = view() % get_replica()->num_of_replicas();
  ...
  if (check_digest())
  {
    ...
    if (is_signed())
    {
      if (!get_node()->get_principal(sender)->verify_signature(
            rep().digest.digest(),
    ...
    if (d == rep().digest)
    {
      Principal* ps = get_node()->get_principal(sender);
      if (!ps)
      {
        return false;
      }
  ...
  Request& req, bool& is_request_present)
{
  ...
  {
    Request* r =
      get_replica()->big_reqs()->lookup(msg->big_req_digest(big_req));
    big_req++;
    if (r == 0)
    {
