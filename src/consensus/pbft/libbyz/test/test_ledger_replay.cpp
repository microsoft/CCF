...
public:
  ...
  ExecCommand exec_command = [](
                               ...
                               ByzInfo& info) {
    outb.contents = get_replica()->create_response_message(client, rid, 0);
    outb.size = 0;
...
TEST_CASE("Test Ledger Replay")
{
  ...
  get_replica()->register_exec(exec_mock.exec_command);
  ...
  INFO("Read the ledger entries and replay them out of order and in order");
  {
    ...
    CHECK(!get_replica()->apply_ledger_data(vec_5_9));
    ...
    for (size_t i = 3; i < total_requests - 1; i += 4)
    {
      ...
      CHECK(get_replica()->apply_ledger_data(vec));
    }
