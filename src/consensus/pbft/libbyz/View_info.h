...
{
  return last_nvs[get_node()->primary(v)].fetch_request(n, d);
}

inline bool View_info::has_complete_new_view(View vi) const
{
  return last_nvs[get_node()->primary(vi)].complete();
}

inline bool View_info::has_nv_message(View vi) const
{
  if (last_nvs[get_node()->primary(vi)].view())
    return true;
  return false;
}
...
{
  PBFT_ASSERT(get_node()->is_replica(id), "Invalid argument");
  return my_vacks[id];
}

inline void View_info::add_missing(Pre_prepare* pp)
{
  last_nvs[get_node()->primary(view())].add_missing(pp);
}

inline void View_info::add_missing(Digest& rd, Seqno n, int i)
{
  last_nvs[get_node()->primary(view())].add_missing(rd, n, i);
}

inline void View_info::add_missing(Prepare* p)
{
  last_nvs[get_node()->primary(view())].add_missing(p);
}
...
inline View_info::OReq_info::OReq_info()
{
  ...
  v = -1;
  lv = -1;
  ods = new ODigest_info[get_node()->f() + 2];
}
