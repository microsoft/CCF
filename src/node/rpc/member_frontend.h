  ...
  public:
    ...
    void init_handlers() override
    {
      ...
      auto propose = [this](EndpointContext& ctx, nlohmann::json&& params) {
        ...
        // caller_identity.request_digest is set when getting the MemberSignatureAuthnIdentity
        // identity. The proposal id is a digest of the root of the state tree at the read version
        // and the request digest.
        std::vector<uint8_t> acc(root_at_read.value().h.begin(), root_at_read.value().h.end());
        acc.insert(acc.end(), caller_identity.request_digest.begin(), caller_identity.request_digest.end());
        const crypto::Sha256Hash proposal_digest(acc);
        const auto proposal_id = proposal_digest.hex_str();
