...
namespace ccf
{
   ...
   * auto foo = json_adapter([](StoreTx& tx, nlohmann::json&& params)
   * {
   *    auto result = fn(params);
   *    if (is_error(result))
   *    {
  ...
  using HandlerTxOnly =
    std::function<details::JsonAdapterResponse(StoreTx& tx)>;
  ...
  using HandlerJsonParamsOnly = std::function<details::JsonAdapterResponse(
    StoreTx& tx, nlohmann::json&& params)>;
  ...
  using HandlerJsonParamsAndCallerId =
    std::function<details::JsonAdapterResponse(
      StoreTx& tx, CallerId caller_id, nlohmann::json&& params)>;
