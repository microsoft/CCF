...
namespace ccf
{
  ...
  public:
    ...
    {
      FORCE_SNAPSHOT_AT_NEXT_SIGNATURE = 0x01
    };

    kv::Version forced_snapshot_after_idx = -1;
  ...
  public:
    ...
    void commit(consensus::Index idx, bool generate_snapshot)
    {
      ...
      bool forced_snapshot = idx > forced_snapshot_after_idx &&
        flag_enabled_unsafe(Flag::FORCE_SNAPSHOT_AT_NEXT_SIGNATURE);
      auto snapshot_idx = forced_snapshot ? idx : next_snapshot_indices.front();
      if (
        snapshot_idx - last_snapshot_idx >= snapshot_tx_interval ||
        forced_snapshot)
      {
        if (
          snapshot_generation_enabled &&
          ((generate_snapshot && snapshot_idx) || forced_snapshot))
        {
          if (forced_snapshot)
          {
            LOG_DEBUG_FMT("Forcing snapshot at {}", snapshot_idx);
          }
          auto msg =
    ...
    bool flag_enabled(Flag f)
    {
      std::lock_guard<std::mutex> guard(lock);
      return flag_enabled_unsafe(f);
    }

    void set_flag_unsafe(Flag f)
    {
      flags |= static_cast<uint8_t>(f);
    }

    void unset_flag_unsafe(Flag f)
    {
      flags &= ~static_cast<uint8_t>(f);
    }

    bool flag_enabled_unsafe(Flag f)
    {
