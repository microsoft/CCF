...
namespace ccf::v8_tmpl
{
  ...
  {
    return static_cast<KVMapHandle*>(obj->GetAlignedPointerFromInternalField(0));
  }

  static void js_kv_map_size_getter(v8::Local<v8::String> property, const v8::PropertyCallbackInfo<v8::Value>& info)
  {
    ...
    v8::Local<v8::Number> value = v8::Number::New(isolate, static_cast<double>(size));
  ...
  static void js_kv_map_get(const v8::FunctionCallbackInfo<v8::Value>& info)
  {
    ...
    if (!val.has_value())
      ...
      value = v8_util::to_v8_array_buffer_copy(isolate, val->data(), val->size());
    info.GetReturnValue().Set(value);
  }
  ...
  static void js_kv_get_version_of_previous_write(const v8::FunctionCallbackInfo<v8::Value>& info)
  {
    v8::Isolate* isolate = info.GetIsolate();
    KVMapHandle* handle = unwrap_kv_map_handle(info.Holder());
    ...
    else
    {
      if (val.value() > v8_util::MAX_SAFE_INTEGER)
      {
        ...
        v8_util::throw_error(isolate,
          fmt::format(
            "Version ({}) is too large to represent as Number object", val.value())
        );
        return;
      }
  ...
  static void js_kv_map_set_read_only(const v8::FunctionCallbackInfo<v8::Value>& info)
  {
    v8_util::throw_type_error(info.GetIsolate(),
      "Cannot call set on read-only map");
  }

  static void js_kv_map_delete_read_only(const v8::FunctionCallbackInfo<v8::Value>& info)
  {
    v8_util::throw_type_error(info.GetIsolate(),
      "Cannot call delete on read-only map");
  }

  static void js_kv_map_clear_read_only(const v8::FunctionCallbackInfo<v8::Value>& info)
  {
    v8_util::throw_type_error(info.GetIsolate(),
      "Cannot call clear on read-only map");
  }

  static v8::Local<v8::ObjectTemplate> create_kv_map_template(v8::Isolate* isolate, bool read_only)
  {
    ...
    tmpl->SetAccessor(
      v8_util::to_v8_istr(isolate, "size"),
  ...
  v8::Local<v8::ObjectTemplate> KVMapReadOnly::create_template(v8::Isolate* isolate)
  {
    return create_kv_map_template(isolate, true);
  }

  v8::Local<v8::ObjectTemplate> KVMapReadWrite::create_template(v8::Isolate* isolate)
  {
    return create_kv_map_template(isolate, false);
  }

  template <typename T>
  static v8::Local<v8::Object> wrap_kv_map(v8::Local<v8::Context> context, KVMapHandle& map_handle)
  {
    ...
    result->SetAlignedPointerInInternalField(0, (void*)&map_handle);

    return handle_scope.Escape(result);
  }

  v8::Local<v8::Object> KVMapReadOnly::wrap(v8::Local<v8::Context> context, KVMapHandle& map_handle)
  {
    return wrap_kv_map<KVMapReadOnly>(context, map_handle);
  }

  v8::Local<v8::Object> KVMapReadWrite::wrap(v8::Local<v8::Context> context, KVMapHandle& map_handle)
  {
