...
namespace ccf::v8_tmpl
{
  ...
  {
    return static_cast<ccf::historical::AbstractStateCache*>(obj->GetAlignedPointerFromInternalField(0));
  }
  ...
  static void get_state_range(const v8::FunctionCallbackInfo<v8::Value>& info)
  {
    ...
    if (!arg1->IsNumber() || !arg2->IsNumber() || !arg3->IsNumber() || !arg4->IsNumber())
    {
    ...
    if (!end_seqno_v8->IntegerValue(context).To(&end_seqno))
      return;
    if (!seconds_until_expiry_v8->IntegerValue(context).To(&seconds_until_expiry))
      return;
    if (
      handle < 0 || start_seqno < 0 || end_seqno < 0 ||
      seconds_until_expiry < 0)
    {
      v8_util::throw_range_error(isolate, "Invalid handle or seqno or expiry: cannot be negative");
      return;
    }
    ...
    v8::Local<v8::Array> v8_states_array = v8::Array::New(isolate,
      v8_states.data(), v8_states.size());

    info.GetReturnValue().Set(v8_states_array);
  }

  static void drop_cached_states(const v8::FunctionCallbackInfo<v8::Value>& info)
  {
    v8::Isolate* isolate = info.GetIsolate();
    v8::Local<v8::Context> context = isolate->GetCurrentContext();
    ccf::historical::AbstractStateCache* state_cache = unwrap_state_cache(info.Holder());
  ...
  v8::Local<v8::ObjectTemplate> Historical::create_template(v8::Isolate* isolate)
  {
  ...
  v8::Local<v8::Object> Historical::wrap(v8::Local<v8::Context> context, ccf::historical::AbstractStateCache* state_cache)
  {
    v8::Isolate* isolate = context->GetIsolate();
    v8::EscapableHandleScope handle_scope(isolate);

    v8::Local<v8::ObjectTemplate> tmpl = get_cached_object_template<Historical>(isolate);
