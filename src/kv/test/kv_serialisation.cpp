...
TEST_CASE(
  "Whole-map dependency ledger encoding compatibility" *
  doctest::test_suite("serialisation"))
{
  using namespace ccf::kv;
  auto encryptor = std::make_shared<NullTxEncryptor>();
  for (const auto domain : {SecurityDomain::PUBLIC, SecurityDomain::PRIVATE})
  {
    Store store;
    const std::string name = domain == SecurityDomain::PUBLIC ?
      "public:compatibility" :
      "compatibility";
    INFO("Map: ", name);
    untyped::Map map(&store, name, domain);
    auto changes = map.create_change_set(0, false);
    REQUIRE(changes != nullptr);
    const SerialisedKey key = {'k'};
    const SerialisedKey removed_key = {'r'};
    const SerialisedKey missing_key = {'m'};
    const SerialisedValue value = {'v'};
    changes->writes[key] = value;
    changes->writes[removed_key] = std::nullopt;

    const auto legacy_entry = [&](
                                Version write_version,
                                Version read_version,
                                const untyped::Read& reads) {
      RawWriter public_writer;
      RawWriter private_writer;
      public_writer.append(EntryType::WriteSet);
      public_writer.append(write_version);
      public_writer.append(NoVersion);
      auto& writer =
        domain == SecurityDomain::PUBLIC ? public_writer : private_writer;
      writer.append(map.get_name());
      writer.append(read_version);
      writer.append(static_cast<uint64_t>(reads.size()));
      for (const auto& [read_key, versions] : reads)
      {
        writer.append(read_key);
        writer.append(std::get<0>(versions));
      }
      writer.append(uint64_t{1});
      writer.append(key);
      writer.append(value);
      writer.append(uint64_t{1});
      writer.append(removed_key);
      RawKvStoreSerialiser serialiser(
        encryptor, {0, write_version}, EntryType::WriteSet, 0);
      return serialiser.serialise_domains(
        public_writer.get_raw_data(), private_writer.get_raw_data());
    };
    const auto serialise = [&]() {
      RawKvStoreSerialiser serialiser(
        encryptor, {0, 1}, EntryType::WriteSet, 0);
      map.serialise_changes(changes.get(), serialiser);
      SizeKvStoreSerialiser size_serialiser(
        encryptor, {0, 1}, EntryType::WriteSet, 0);
      map.serialise_changes(changes.get(), size_serialiser);
      auto bytes = serialiser.get_raw_data();
      REQUIRE(size_serialiser.get_serialised_size() == bytes.size());
      return bytes;
    };

    REQUIRE(changes->read_version == NoVersion);
    const auto legacy = legacy_entry(1, NoVersion, {});
    REQUIRE(serialise() == legacy);

    untyped::MapHandle handle(*changes, map.get_name());
    REQUIRE_FALSE(handle.get(missing_key).has_value());
    const auto key_reads = changes->reads;
    REQUIRE(key_reads.contains(missing_key));
    REQUIRE(serialise() == legacy);
    handle.foreach([](const auto&, const auto&) { return true; });
    REQUIRE(changes->read_version == 0);
    REQUIRE(serialise() == legacy);
    REQUIRE(changes->read_version == 0);

    changes->read_version = 7;
    REQUIRE(serialise() == legacy);
    REQUIRE(changes->read_version == 7);
    REQUIRE(changes->reads == key_reads);

    const untyped::Read legacy_reads = {
      {key, {7, NoVersion}}, {missing_key, {NoVersion, NoVersion}}};
    for (const auto read_version : {NoVersion, Version{7}})
    {
      for (const auto& reads : {untyped::Read{}, legacy_reads})
      {
        const auto bytes = legacy_entry(1, read_version, reads);
        RawKvStoreDeserialiser deserialiser(encryptor);
        Term term = 0;
        EntryFlags flags = {};
        REQUIRE(
          deserialiser.init(bytes.data(), bytes.size(), term, flags, false) ==
          1);
        REQUIRE(deserialiser.start_map() == map.get_name());
        auto decoded = map.deserialise_internal(deserialiser, 0);
        REQUIRE(decoded->writes == changes->writes);
        REQUIRE(decoded->reads == reads);
        REQUIRE(deserialiser.end());
        REQUIRE(decoded->read_version == read_version);
      }
    }

    Store restored;
    restored.set_encryptor(encryptor);
    REQUIRE(restored.deserialize(legacy)->apply() == ApplyResult::PASS);
    const untyped::Read replay_reads = {
      {key, {1, NoVersion}}, {missing_key, {NoVersion, NoVersion}}};
    REQUIRE(
      restored.deserialize(legacy_entry(2, 1, replay_reads))->apply() ==
      ApplyResult::PASS);
    REQUIRE(
      restored.deserialize(legacy_entry(3, NoVersion, {}))->apply() ==
      ApplyResult::PASS);
    auto reader = restored.create_tx();
    auto* restored_map = reader.ro<untyped::Map>(map.get_name());
    REQUIRE(restored_map->get(key) == value);
    REQUIRE_FALSE(restored_map->has(removed_key));
    REQUIRE(restored_map->get_version_of_previous_write(key) == 3);
  }
}
