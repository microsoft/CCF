...

  {
    ...
    REQUIRE(
      kv_store_target
        ...
        ->apply(false) == kv::ApplyResult::PASS);
  ...
  doctest::test_suite("serialisation"))
{
  ...
  INFO("Deserialise transaction in target store");
  {
    ...
    REQUIRE(
      kv_store_target.deserialize(latest_data.value(), ConsensusType::CFT)
        ->apply(false) != kv::ApplyResult::FAIL);
  ...
  VerboseSerialisedMap)
{
  ...
  INFO("Serialise/Deserialise 2 kv stores");
  {
    ...
    REQUIRE(
      kv_store2.deserialize(data, ConsensusType::CFT)->apply(false) ==
      kv::ApplyResult::PASS);
...
TEST_CASE("nlohmann (de)serialisation" * doctest::test_suite("serialisation"))
{
  ...
  SUBCASE("baseline")
  {
    ...
    REQUIRE(latest_data.has_value());
    REQUIRE(
      s1.deserialize(latest_data.value(), ConsensusType::CFT)->apply(false) !=
      kv::ApplyResult::FAIL);
  }
  ...
  doctest::test_suite("serialisation"))
{
...

  {
    ...
    {
      REQUIRE(
        kv_store_target.deserialize(data, ConsensusType::CFT)->apply(false) ==
        kv::ApplyResult::PASS);
  ...
  doctest::test_suite("serialisation"))
{
  ...
  INFO("Deserialise transaction in target store and extract claims");
  {
    ...
    auto wrapper =
      kv_store_target.deserialize(latest_data.value(), ConsensusType::CFT);
    REQUIRE(wrapper->apply(false) != kv::ApplyResult::FAIL);
