...
namespace ccf::kv
{
  ...
  public:
    ...
    PendingTxInfo commit_reserved(
      PostApplyObserver before_prepare_observer = nullptr)
    {
      if (committed)
      {
        throw std::logic_error("Transaction already committed");
      ...
      // rollback, so that a discarded signature leaves no marker behind.
      if (before_prepare_observer != nullptr)
      {
        before_prepare_observer();
      }
