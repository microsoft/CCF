...
namespace crypto
{
  ...
  static mbedtls_md_type_t get_md_type(MDType mdt)
  {
    switch (mdt)
    {
      case MDType::NONE:
  ...
  std::string PublicKey_mbedTLS::error_string(int err)
  {
  ...
  static void MCHK(int rc)
  {
    ...
    {
      throw std::logic_error(
        fmt::format("mbedTLS error: {}", PublicKey_mbedTLS::error_string(rc)));
    }
  }
