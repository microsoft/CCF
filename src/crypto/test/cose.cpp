...
TEST_CASE("Decode CCF COSE receipt")
{
  ...
  for (const auto size : {size_t{0}, size_t{1}, size_t{31}, size_t{33}})
  {
    const auto malformed = with_proof_hash_size(ProofHashField::Sibling, size);
    REQUIRE_THROWS_AS(
      ccf::cose::decode_ccf_receipt(malformed, true),
      ccf::cose::COSEDecodeError);
  }

  const auto valid_sibling = with_proof_hash_size(ProofHashField::Sibling, 32);
  REQUIRE_NOTHROW(ccf::cose::decode_ccf_receipt(valid_sibling, true));
