...
TEST_CASE("CBOR: signed integers")
{
  ...
  for (const auto& [name, hex, expected_value, expected_repr] : test_cases)
  {
    SUBCASE(name)
    {
      ...
      auto value = nondet_parse(cbor_bytes);

      REQUIRE(value.as_signed() == expected_value);

      auto encoded = value.nondet_serialize();
      auto decoded = nondet_parse(encoded);
      REQUIRE_EQ(cbor_bytes, encoded);

      REQUIRE(decoded.as_signed() == expected_value);

      const std::string result = ccf::cbor::test::to_string(value);
...
TEST_CASE("CBOR: signed integer overflow")
{
  ...
  REQUIRE_THROWS_AS(nondet_parse(cbor_bytes), DecodeError);
}
...
TEST_CASE("CBOR: array [1, 2, 3, 4, 5]")
{
  ...
  {
    REQUIRE(arr.array_at(i).as_signed() == i + 1);
  }
...
TEST_CASE("CBOR: array ['a', 'b', 'c']")
{
  ...
  REQUIRE(arr.array_at(0).as_string() == "a");
  REQUIRE(arr.array_at(1).as_string() == "b");
  REQUIRE(arr.array_at(2).as_string() == "c");
...
TEST_CASE("CBOR: array [b'x', b'y', b'z']")
{
  ...
  auto bytes0 = arr.array_at(0).as_bytes();
  REQUIRE(bytes0.size() == 1);
  REQUIRE(bytes0[0] == 'x');

  auto bytes1 = arr.array_at(1).as_bytes();
  REQUIRE(bytes1.size() == 1);
  REQUIRE(bytes1[0] == 'y');

  auto bytes2 = arr.array_at(2).as_bytes();
...
TEST_CASE("CBOR: array [True, False, None]")
{
  ...
  REQUIRE(arr.array_at(0).as_simple() == SimpleValue::True);
  REQUIRE(arr.array_at(1).as_simple() == SimpleValue::False);
  REQUIRE(arr.array_at(2).as_simple() == SimpleValue::Null);
...
TEST_CASE("CBOR: array [1, 'two', b'3', True, None]")
{
  ...
  REQUIRE(arr.array_at(1).as_string() == "two");

  auto bytes = arr.array_at(2).as_bytes();
  REQUIRE(bytes.size() == 1);
  REQUIRE(bytes[0] == '3');

  REQUIRE(arr.array_at(3).as_simple() == SimpleValue::True);

  REQUIRE(arr.array_at(4).as_simple() == SimpleValue::Null);
...
TEST_CASE("CBOR: array [0, -1, 42, -100, 65535]")
{
  ...
  REQUIRE(arr.array_at(0).as_signed() == 0);

  REQUIRE(arr.array_at(1).as_signed() == -1);

  REQUIRE(arr.array_at(2).as_signed() == 42);

  REQUIRE(arr.array_at(3).as_signed() == -100);

  REQUIRE(arr.array_at(4).as_signed() == 65535);
...
TEST_CASE("CBOR: nested array [[1, 2], [3, 4], [5, 6]]")
{
  ...
  {
    const auto inner = arr.array_at(i);
    REQUIRE(inner.size() == 2);

    REQUIRE(inner.array_at(0).as_signed() == i * 2 + 1);
    REQUIRE(inner.array_at(1).as_signed() == i * 2 + 2);
  }
...
TEST_CASE("CBOR: deeply nested array [1, [2, 3], 4, [5, [6, 7]]]")
{
  ...
  const auto arr1 = arr.array_at(1);
  REQUIRE(arr1.size() == 2);
  REQUIRE(arr1.array_at(0).as_signed() == 2);
  REQUIRE(arr1.array_at(1).as_signed() == 3);

  REQUIRE(arr.array_at(2).as_signed() == 4);

  const auto arr3 = arr.array_at(3);
  REQUIRE(arr3.size() == 2);
  REQUIRE(arr3.array_at(0).as_signed() == 5);

  const auto arr3_1 = arr3.array_at(1);
  REQUIRE(arr3_1.size() == 2);
  REQUIRE(arr3_1.array_at(0).as_signed() == 6);
  REQUIRE(arr3_1.array_at(1).as_signed() == 7);
...
TEST_CASE("CBOR: tagged empty array Tag(9300) with []")
{
  ...
  REQUIRE_THROWS_AS((void)array.array_at(0), DecodeError);
...
TEST_CASE("CBOR: map {'a': 1, 'b': 2, 'c': 3}")
{
  ...
  const auto& map = value;
  REQUIRE(map.size() == 3);

  REQUIRE(map.map_key_at(0).as_string() == "a");
  REQUIRE(map.map_value_at(0).as_signed() == 1);
  REQUIRE(map.map_key_at(1).as_string() == "b");
  REQUIRE(map.map_value_at(1).as_signed() == 2);
  REQUIRE(map.map_key_at(2).as_string() == "c");
  REQUIRE(map.map_value_at(2).as_signed() == 3);
...
TEST_CASE("CBOR: map {'x': 'y', 'foo': 'bar'}")
{
  ...
  REQUIRE(map.map_key_at(0).as_string() == "x");
  REQUIRE(map.map_value_at(0).as_string() == "y");
  REQUIRE(map.map_key_at(1).as_string() == "foo");
  REQUIRE(map.map_value_at(1).as_string() == "bar");
...
TEST_CASE("CBOR: map {'enabled': True, 'disabled': False, 'unknown': None}")
{
  ...
  REQUIRE(map.map_key_at(0).as_string() == "enabled");
  REQUIRE(map.map_value_at(0).as_simple() == SimpleValue::True);
  REQUIRE(map.map_key_at(1).as_string() == "disabled");
  REQUIRE(map.map_value_at(1).as_simple() == SimpleValue::False);
  REQUIRE(map.map_key_at(2).as_string() == "unknown");
  REQUIRE(map.map_value_at(2).as_simple() == SimpleValue::Null);
...
TEST_CASE("CBOR: map with mixed key types")
{
  ...
  REQUIRE(value.map_at(make_bytes(bytes_key)).as_simple() == SimpleValue::True);
...
TEST_CASE("CBOR: encode empty bytes and string with null data pointer")
{
  ...
  const std::span<const uint8_t> null_bytes{
    static_cast<const uint8_t*>(nullptr), 0};
  REQUIRE_EQ(
    make_bytes(null_bytes).nondet_serialize(), ccf::ds::from_hex("40"));

  const std::string_view null_string{};
  REQUIRE(null_string.data() == nullptr);
  REQUIRE_EQ(
    make_string(null_string).nondet_serialize(), ccf::ds::from_hex("60"));

  // The common case: an empty vector, whose data() may be null.
  const std::vector<uint8_t> empty_vec;
  REQUIRE_EQ(make_bytes(empty_vec).nondet_serialize(), ccf::ds::from_hex("40"));
}
...
TEST_CASE("CBOR: error - array out of bounds")
{
  ...
  auto value = nondet_parse(cbor_bytes);
  REQUIRE_THROWS_AS((void)value.array_at(10), DecodeError);
}
...
TEST_CASE("CBOR: error - unexpected tag")
{
  ...
  REQUIRE_THROWS_AS((void)value.tag_at(9004), DecodeError);
}
...
TEST_CASE("CBOR: throw with context")
{
  ...
  // Accessors report the operation that failed.
  const std::string err = "as_string";
...
TEST_CASE("CBOR: parse max depth")
{
  ...
  REQUIRE_NOTHROW(nondet_parse(depth1, 2));

  // depth 2: [[42]] -- should pass at max_depth=2
  auto depth2 = ccf::ds::from_hex("8181182a");
  REQUIRE_NOTHROW(nondet_parse(depth2, 2));

  // depth 3: [[[42]]] -- should fail at max_depth=2
  auto depth3 = ccf::ds::from_hex("818181182a");
  REQUIRE_THROWS_AS(nondet_parse(depth3, 2), DecodeError);

  // map depth 3: {1: {1: {1: 42}}} -- should fail at max_depth=2
  auto map_depth3 = ccf::ds::from_hex("a101a101a101182a");
  REQUIRE_THROWS_AS(nondet_parse(map_depth3, 2), DecodeError);

  // map depth 2: {1: {1: 42}} -- should pass at max_depth=2
  auto map_depth2 = ccf::ds::from_hex("a101a101182a");
  REQUIRE_NOTHROW(nondet_parse(map_depth2, 2));
}

TEST_CASE("CBOR: serialize max depth")
{
  // depth 1: [42] -- should pass at max_depth=1,2
  std::vector<Value> shallow_items;
  shallow_items.push_back(make_signed(42));
  auto shallow = make_array(std::move(shallow_items));
  REQUIRE_NOTHROW(std::ignore = shallow.nondet_serialize(1));
  REQUIRE_NOTHROW(std::ignore = shallow.nondet_serialize(2));

  // Build 3 levels: [[[42]]]
  auto deep = make_signed(42);
  for (int i = 0; i < 3; ++i)
  {
    std::vector<Value> level;
    level.push_back(std::move(deep));
    deep = make_array(std::move(level));
  }
  REQUIRE_THROWS_AS(std::ignore = deep.nondet_serialize(2), EncodeError);

  // Same deep tree passes at higher limit
  REQUIRE_NOTHROW(std::ignore = deep.nondet_serialize(3));
  REQUIRE_NOTHROW(std::ignore = deep.nondet_serialize(4));
}
