...
namespace ccf::http
{
  ParsedQuery parse_query(const std::string_view& query)
  {
    ...
    {
      if (
        query[pos] == '%' && pos + 2 < query.length() &&
        (std::isxdigit(query[pos + 1]) != 0) &&
        (std::isxdigit(query[pos + 2]) != 0))
      {
    ...
    for (const auto& param : params)
    {
      ...
      {
        if (
          param[i] == '%' && i + 2 < param.length() &&
          (std::isxdigit(param[i + 1]) != 0) &&
          (std::isxdigit(param[i + 2]) != 0))
        {
          // Skip URL-encoded sequence
          i += 3;
        }
