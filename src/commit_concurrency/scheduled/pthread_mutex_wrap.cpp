...
// ccf::ds::Mutex - used, unmodified, by production code linked into this
...
//
// ccf::ds::Mutex's own lock()/try_lock()/unlock() (see
// include/ccf/ds/locking.h) set ccf::ds::detail::pending immediately
// before making the real call intercepted here. This is how the
// functions below tell a genuine ccf::ds::Mutex call apart from every
...
namespace
{
  ...
  {
    if (!ccf::ds::detail::pending)
    {
      return false;
    }
    ccf::ds::detail::pending = false;
    return true;
  }
}
