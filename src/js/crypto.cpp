...
#pragma clang diagnostic ignored "-Wc99-extensions"
      ...
      JSContext* ctx, JSValueConst, int argc, JSValueConst* argv)
    {
      ...
      auto wrap_algo_name_cstr =
        auto_free(JS_ToCString(ctx, wrap_algo_name_val));
      ...
      try
      {
        ...
        if (algo_name == "RSA-OAEP")
        {
          ...
          auto label_val =
            auto_free(JS_GetPropertyStr(ctx, parameters, "label"));
          size_t label_buf_size = 0;
          uint8_t* label_buf =
            JS_GetArrayBuffer(ctx, &label_buf_size, label_val);
