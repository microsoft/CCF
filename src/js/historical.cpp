...
#pragma clang diagnostic ignored "-Wc99-extensions"
  ...
  static JSValue ccf_receipt_to_js(JSContext* ctx, ccf::historical::TxReceiptPtr receipt)
  {
  ...
  {
    auto* state_ctx = (HistoricalStateContext*)JS_GetOpaque(val, historical_state_class_id);
    delete state_ctx;
  }
    ...
    JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv)
  {
    ...
    if (JS_ToInt64(ctx, &seconds_until_expiry, argv[3]) < 0)
      return JS_EXCEPTION;
    if (handle < 0 || start_seqno < 0 || end_seqno < 0 || seconds_until_expiry < 0)
      return JS_ThrowRangeError(ctx, "Invalid handle or seqno or expiry: cannot be negative");
    ...
    try
    {
      states = historical_state->get_state_range(handle, start_seqno, end_seqno, std::chrono::seconds(seconds_until_expiry));
    }
