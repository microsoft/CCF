...
#include "node/rpc/rpcexception.h"
  ...
  namespace lua
  {
    ...
    protected:
      ...
      private:
        ...
        template <typename T>
        static void add_table(lua::Interpreter& li, Store::Tx& tx, T& table)
        {
        ...
        static void process_tables(
          lua::Interpreter&, Store::Tx&, const Whitelist&)
        {}
      ...
      public:
        template <typename T>
        static void create(
          lua::Interpreter& li, Store::Tx& tx, const std::vector<T*>& tables)
        {
          register_meta<T>(li);
      ...
      Whitelist get_whitelist(Store::Tx& tx, WlId id) const
      {
    ...
    public:
       ...
       * -- read members table
       * local member_0 = tables_readable.members:get(0)
       *
