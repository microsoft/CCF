...
//       {
//         LOG_FAIL_FMT(
//           "Exception thrown while executing post-task callback: {}",
//           e.what());
//       }
...
//   TaskHandle TaskSystem::enqueue_task(std::unique_ptr<ccf::tasks::Task>&&
//   task)
//   {
...
//     else
//     {
//       // uv_queue_work cannot be called directly from the thread pool, so
//       need
...
//     // Enqueue a new task (handling the dispatch to the main thread if
//     required) return enqueue_task(std::make_unique<SimpleTask>(
//       // Do nothing in the main body of that task
//       []() {},
//       // But in the completion callback, when we know we're on the main
//       thread
