...
namespace ccf::uv::tasks
{
  namespace
  {
    ...
    {
      auto task = static_cast<ccf::uv::tasks::Task*>(req->data);
    ...
    static ccf::tasks::LockingConcurrentQueue<uv_work_t*> pending_work = {};
  ...
  TaskHandle TaskSystem::enqueue_task(
    ...
    const std::chrono::milliseconds& delay)
  {
    ...
    return enqueue_task(std::make_unique<SimpleTask>(
      ...
      []() {},
      // But in the completion callback, when we know we're on the mainthread
