...
CCF comes with a generic application for running Lua scripts called `luageneric`, implemented in `luageneric.cpp <https://github.com/microsoft/CCF/blob/master/src/apps/lua_generic/luageneric.cpp>`_. At runtime, *luageneric* dispatches incoming RPCs to Lua scripts stored in the table *APP_SCRIPTS*. The RPC method name is used as the key, and if a script exists at this key it is called with the RPC arguments.
