...
Each transaction listed in the scenario is then sent to either the `primary` or a `backup` node. As the current implementation uses Raft for consensus, any ``Write`` transactions (which modify the KV) must be sent to the `primary`. Sending them to a `backup` will result in a ``TX_NOT_PRIMARY`` error response. ``Read`` transactions can sent to any node.
