...
First, the new member should update and retrieve the latest state digest via the ``gov/ack/update_state_digest`` command. In doing so, the new member confirms that they are satisfied with the current state of the service.
...
Then, the new member should sign the state digest returned by the ``gov/ack/update_state_digest`` via the ``gov/ack`` command, using the ``scurl.sh`` utility:
