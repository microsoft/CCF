...
.. note:: A CCF node started with existing ledger files may suffix some of the file names with ``.corrupted`` (ledger file cannot be parsed) on startup, depending on the sequence number the node will join from.
