...
For an open service, a downloaded COSE snapshot may be signed by an earlier service identity. The joiner obtains the endorsement chain from :http:GET:`/node/snapshot/{snapshot_name}/endorsements` and verifies it against ``command.join.service_certificate_file``. Each signature and service-epoch boundary must be valid, and the endorsed epoch must cover the snapshot. The endpoint returns a JSON array of base64-encoded COSE endorsements, newest first, and requires the ``SnapshotRead`` operator feature.

Snapshot endorsement chains are limited to 64 links, 1 MiB per endorsement, and 4 MiB in total before base64 encoding. Unavailable, invalid, or excessive chains cause verification to fail; they do not relax the configured trust anchor. Recovery-time snapshot fallback is separate: the new service's endorsement of its predecessor is not available until recovery completes.
