...
Time-based snapshotting can also be enabled with ``snapshots.time_interval``. When this is set, a snapshot is eligible once more than ``snapshots.min_tx_count`` transactions have elapsed since the last non-forced snapshot. These transactions include CCF internal signature and snapshot bookkeeping transactions as well as application writes.
