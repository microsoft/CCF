...
The constitution for a CCF service is implemented as a set of JS scripts. These scripts can be submitted at network startup in the ``command.start.constitution_files`` configuration entry, or updated by a governance proposal. They will be concatenated into a single entry in the ``public:ccf.gov.constitution`` table, and should export 3 named functions:
    ...
    - ``resolve``: This takes the raw proposal, proposer ID, ballot results, and proposal ID, and determines whether the proposal should be accepted, rejected, or remain open. In the simple case this might simply accept proposals after a majority of members have voted in favour. It could also examine member data to give each member a different role or weight, or have different thresholds for each action. This has read-only access to the KV.
    - ``apply``: This takes a proposal which has been accepted by ``resolve`` and its proposal ID, and should make the proposed changes to the service's state. For instance if the proposal added a new user this should extract their cert and data from the proposal and write them to the appropriate KV tables. This has full read-write access to the KV.

The ``ccf.gov.validateConstitution`` API is available only during the constitution's ``validate`` step. It checks that a proposed constitution is a non-empty, parseable string exporting ``validate``, ``resolve``, and ``apply`` functions with the expected numbers of arguments. The proposed constitution is evaluated in an isolated interpreter, subject to the same heap, stack and execution time limits (see :doc:`js_runtime`) as the calling ``validate`` step, and sharing its remaining execution time. Exceeding these limits fails validation. Other exceptions thrown at module scope are not checked, since the isolated interpreter does not provide the ``ccf`` or ``console`` APIs. The default constitution uses it to validate ``set_constitution`` proposals. It is not available to application code, ballots, or the constitution's ``resolve`` and ``apply`` steps.
