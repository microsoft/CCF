...
.. note::
    ...
    If auto-refresh fails with a TLS certificate verification error, make sure the IdP's issuing root CA is installed in the trust store of the host or container image running the CCF node, then restart or redeploy the node so outbound fetches can use the updated trust store.
