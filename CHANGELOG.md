...
- Completing recovery no longer degrades ledger write latency on Azure Files/SMB (CIFS) mounts whose client kernel lacks the compound-operation lease-key fix (e.g. Linux 6.1). Active (uncommitted) recovery ledger chunks are now closed before their temporary `.recovery` suffix is removed and reopened immediately afterwards, so the writable handle reacquires a fresh write-caching lease instead of writing through to the server on every subsequent entry (#TBD).
