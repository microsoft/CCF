...
- JavaScript `verifySnpAttestation()` returned swapped `current_minor` and `current_build` values. They now match the AMD SEV-SNP report layout. The deprecated C++ `ccf::pal::snp::Attestation` keeps its previous field mapping (#8083).
...
- Azure cache and THIM endorsement requests now encode the reported TCB as 16 hexadecimal digits, preserving leading zeroes (#8083).
