...
- Added a `consensus.max_uncommitted_tx_count` configuration option, which if set specifies the maximum number of transactions that can be pending on the primary. When that threshold is exceeded, a `503 Service Unavailable` is temporarily returned on all but the `/node/*` paths (#5692).
