...
- Python ledger signature verification is consolidated in `ccf.signatures`, behind `verify_all_root_signatures()` and a `ROOT_SIGNATURE_VERIFIERS` registry. `ccf.cose.verify_receipt()` has moved and been renamed to `ccf.receipt.verify_cose()`, with no compatibility alias (#8109).
