...
- JWT/JWK auto-refresh outbound TLS verification now uses the node host's system trust store. The `set_ca_cert_bundle` and `remove_ca_cert_bundle` governance actions, `ca_cert_bundle_name` JWT issuer metadata, and `/gov/service/jwk` CA bundle reporting have been removed (#8006).
