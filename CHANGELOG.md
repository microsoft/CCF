...
- C++ formatting now uses `std::format` instead of the bundled `{fmt}` library. Applications using the exported `fmt` headers or custom `fmt::formatter` specializations must migrate to the standard API; see [C++ formatting](https://microsoft.github.io/CCF/main/build_apps/logging.html#c-formatting) for enum, range, and custom-type migration guidance. The C++23 requirement is unchanged (#8368).
