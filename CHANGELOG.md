...
- COSE signatures produced by CCF (ledger signatures, receipts and service identity endorsements) now use the fully-specified ECDSA algorithm identifiers introduced by [RFC 9864](https://www.rfc-editor.org/rfc/rfc9864.html): `ESP256` (-9), `ESP384` (-51) and `ESP512` (-52), instead of the deprecated `ES256` (-7), `ES384` (-35) and `ES512` (-36). With CCF's default P-384 service identity, the `alg` protected header of receipts and ledger signatures changes from -35 to -51. Verification continues to accept both spellings, treating `ESn` and `ESPn` as equivalent for the curve they denote, so existing signatures and client-produced COSE Sign1 requests remain valid. Verifiers which pin a specific `alg` value must be updated to accept the `ESP` identifiers (#8267).
