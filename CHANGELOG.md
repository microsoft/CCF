...
- Recovery procedure:
  - The `GET /gov/recovery_share` endpoint has been renamed to `GET /gov/encrypted_recovery_share/{member_id}`. This can be accessed without any authentication.
  - The `POST /gov/recovery_share` endpoint request must now be signed, and will not allow purely TLS authentication, for parity with other `/gov` endpoints and to reduce use of member certs (members are instead primarily identified purely by their _keys_).
