...
- Added `POST /node/snapshot/force`, gated by the `SnapshotTrigger` RPC interface operator feature, to trigger a snapshot via an operator endpoint rather than a governance action.
