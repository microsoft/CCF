...
- Task exception stacktraces prefer C++23 `std::stacktrace` when supported, with an explicit `CCF_STACKTRACE_BACKEND` build option and a libbacktrace fallback. Azure Linux 4 builds no longer require the Azure Linux 3 libbacktrace RPM. See [Build CCF from Source](https://microsoft.github.io/CCF/main/contribute/build_ccf.html#task-stacktraces) for backend and link requirements.
