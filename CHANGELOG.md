...
- Python ledger signature verification is consolidated in `ccf.signatures`, behind `verify_all_root_signatures()`, which now requires at least one signature to verify. `ccf.cose.verify_receipt()` has moved and been renamed to `ccf.receipt.verify_cose()`, with no compatibility alias (#8109).
