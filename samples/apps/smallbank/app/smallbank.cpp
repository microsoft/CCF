  ...
  public:
      ...
      checkingTable(tables.create<uint64_t, int64_t>("c"))
    {
      auto create = [this](Store::Tx& tx, const nlohmann::json& params) {
        ...
        if (account_r.has_value())
        {
          return jsonrpc::error(
            jsonrpc::ErrorCodes::INVALID_PARAMS, "Account already exists");
        }
      ...
      auto balance = [this](Store::Tx& tx, const nlohmann::json& params) {
        ...
        if (!account_r.has_value())
          return error(
            jsonrpc::ErrorCodes::INVALID_PARAMS, "Account does not exist");
      ...
      auto transact_savings =
        [this](Store::Tx& tx, const nlohmann::json& params) {
          ...
          if (name.empty())
            return error(
              jsonrpc::ErrorCodes::INVALID_PARAMS, "A name must be specified");
      ...
      auto deposit_checking =
        [this](Store::Tx& tx, const nlohmann::json& params) {
          ...
          if (value <= 0)
            return error(jsonrpc::ErrorCodes::INVALID_PARAMS, "Value <= 0");
